...
Besides, the energy consumption of crawl access is non-negligible.
...
Finally a case study is performed with real world data from Xively, showing that EasiCrawl has lower crawling latency and energy consumption than traditional Web-based search scheduling.
...
A large number of sensors have been deployed and connected to the Internet, which assists the human society to sense and control the physical space in an intelligent way. 
The extended Internet with sensors are known as the Internet of Things (IoT). 
With the booming of sensors in IoT, it is an inevitable trend to develop a generic search engine for IoT users and application builders to find different kinds of IoT sensors as required. 
...
IoT crawlers crawl different sensors for these descriptions files, and copy these into a local repository at server side. 
This repository is an organized storage, with which the user queries about IoT sensors can be accomplished.
