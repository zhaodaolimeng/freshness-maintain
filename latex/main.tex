...
\author{
\IEEEauthorblockN{
	Meng Li\IEEEauthorrefmark{1}\IEEEauthorrefmark{2}, 
	Haiming Chen\IEEEauthorrefmark{1}, 
	Xi Huang\IEEEauthorrefmark{1},
	Li Cui\IEEEauthorrefmark{1}}
...
%% Examples
% The search of publicly accessible IoT sensors will unlock the potential of IoT in sharing information, and consequently uncover the value of data in the physical world.
...
% The implementation of this search engine will realized the potential of IoT in sharing information and consequently discover the value of data in the physical world.
...
% In near future, unmanned systems like automatic driving are likely to be heavily dependent on information gathered from nearby sensors or other IoT sensors, which also needs an effective sensor search engine. 
...
% While contain many services and resources, these IoT systems can be both potential data providers or customers of a generic IoT search engine. 
...
% In this architecture, it is assumed that resources can spontaneously provide description files, giving events captured by local sensors.
...
% In real world IoT systems, IoT sensor’s usually work in low \textbf{duty cycle}, which means during their lifetime they switch between the working period and the sleeping period.
...
% Sometimes the importance of different sensors are distinct. 
...
% In this section, at first, we bring forth the framework of EasiCrawl, which iteratively improves a feasible solution to achieve a latency optimal scheduling plan. 
...
% A supportive result is showed in section \ref{simulation} about evaluation with different total crawl limits, when sum of crawls are much smaller than that of work cycles.
% In addition, the computation of $E_h[G(t)]$ and $E_f(t_s,t)$, which are both expected latency of a single sensor, is discussed in the appendix.
