...
We first study performance of methods under different situation, in which simulation events are generated, and latency of events are taken as a final measurement.
Then, we carry out a benchmark of EasiCrawl to test the latency expectation with different input parameters. A comparison is made between EasiCrawl and a random method. We use the latency expectation as a measurement.
All these simulation are implemented on a PC with MATLAB.

\subsection{Benchmark}

Sensor scales $N$, total crawl limit $\theta$, discretization step length $\epsilon$, and time range $T$ of scheduling are tested in this section. A random schedule method is designed for comparison. In this method, crawl number arrangement is set arbitrarily. and the crawl time for each sensor, except the last time node which ensures all the latency expectation is included, is also randomly chosen.

\begin{figure}
	\centering
	\includestandalone[width=\linewidth]{pic/test1_sensorscale}
	\captionsetup{justification=centering}
	\caption{Latency Expectation Under Different Sensor Scales}
	\label{fig:test1_sensorscale}
\end{figure}

\subsection{Performance of Single Sensor Scheduling Method}
