...
Notice method \ref{alg:framework} has not set any specified requirements for the sensors sleep behavior.
However, the latency expectation $E_L(n_i)$ of a single sensor $i$ is closely related to sensors sleep plan.
In the following paper, sensors are classified into 3 types according to their sleep behavior.
To solve the schedule problem for a single sensor, we proposed a dynamic programming method for the optimal scheduling, but find the time complexity is somewhat unacceptable. 
So, a greedy method is designed as a trade-off for time efficiency. 
We also discuss the situation when the sensors' sleep plan is unknown in the following part.
