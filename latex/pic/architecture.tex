...
\node [crawler][below of = r](cw) {IoT Search}
  child {node[level 2] (c1) {    Personal    }}
  child {node[level 2] (c2) {    Local Area    }}
  child {node[level 2] (c3) {    Public    }};
...
\node [below of = c11] (c12) {Motion Detection};

\node [below of = c2, xshift=15pt] (c21) {Energy Monitor};
\node [below of = c21] (c22) {Indoor Environment Monitor};

\node [below of = c3, xshift=15pt] (c31) {Traffic Monitoring};
\node [below of = c31] (c32) {Large Scale Environmental Sensing};
\end{scope}
...
% axis
\draw[thick,<->] (-4.5,-5.5) node[anchor=east]{Privacy} -- (4.5,-5.5) node[anchor=west]{Cooperativity};
\draw[thick,<->] (-6.25,-3.25) node[anchor=south, text width=2cm, align=center]{Initiative\\ Services} -- 
				(-6.25,-4.25) node[anchor=north, text width=2cm, align=center]{Passive\\ Resources};
